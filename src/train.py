...
class CryptoMLPipeline:
    ...
    def __init__(self, data_dir='./data', model_dir='./models', is_colab=False):
        ...
        logger.info(f"初始化pipeline - Colab模式: {is_colab}")
    ...
    def phase3_model_training(self, processed_data, epochs=150, batch_size=32):
        """Phase 3: 模型訓練"""
        logger.info("\n" + "="*60)
        logger.info("Phase 3: 模型訓練（PyTorch + CUDA）")
        logger.info("="*60)
...
if __name__ == '__main__':
    ...
    # 初始化Pipeline
    pipeline = CryptoMLPipeline(is_colab=IS_COLAB)
